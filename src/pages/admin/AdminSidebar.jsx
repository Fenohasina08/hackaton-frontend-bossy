  ...
  return (
    <nav className={`flex flex-col h-full glass transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <ul className="flex-1 space-y-2 overflow-y-auto py-2">
        {[
          { to: "/admin", icon: <LayoutDashboard size={20} />, label: "Stats" },
          { to: "/admin/universities", icon: <BookOpen size={20} />, label: "Universities" },
          { to: "/admin/scholarships", icon: <Compass size={20} />, label: "Scholarships" },
        ].map((item) => (
          <li key={item.to}>
